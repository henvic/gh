    ...
    if (options.label) {
        if (options.delete) {
            hooks.invoke('repo.deleteLabel', instance, function(afterHooksCallback) {
                ...
                logger.log('Deleting label ' + logger.colors.green(options.label) + ' on ' +
                    logger.colors.green(user + '/' + options.repo));

                instance.deleteLabel(function (err1, repo) {
                    if (err1) {
                        logger.error('Can\'t delete label. ' + JSON.parse(err1).message);
                        return;
                    }
                });
            ...
            hooks.invoke('repo.listLabels', instance, function(afterHooksCallback) {
                ...
                if (options.page) {
                    logger.log('Listing labels from page ' + logger.colors.green(options.page) + ' on ' +
                        logger.colors.green(user + '/' + options.repo));
                } else {
                    logger.log('Listing labels' + ' on ' + logger.colors.green(user + '/' + options.repo));
                }

                instance.listLabels(function (err1, repo) {
                    if (err1) {
                        logger.error('Can\'t list labels. ' + JSON.parse(err1).message);
                        return;
                    }
                });
            ...
            hooks.invoke('repo.createLabel', instance, function(afterHooksCallback) {
                ...
                logger.log('Creating label ' + logger.colors.green(options.label) + ' on ' +
                    logger.colors.green(user + '/' + options.repo));

                instance.createLabel(function (err1, repo) {
                    if (err1) {
                        logger.error('Can\'t create label. ' + JSON.parse(err1).message);
                        return;
                    }
                });
            ...
            hooks.invoke('repo.updateLabel', instance, function(afterHooksCallback) {
                ...
                logger.log('Updating label ' + logger.colors.green(options.label) + ' on ' +
                    logger.colors.green(user + '/' + options.repo));

                instance.updateLabel(function (err1, repo) {
                    if (err1) {
                        logger.error('Can\'t update label. ' + JSON.parse(err1).message);
                        return;
                    }
                });
...
Repo.prototype.createLabel = function (opt_callback) {
    var instance = this,
        options = instance.options,
        payload;
    ...
    if (options.organization) {
        payload.user = options.organization;
    }
...
Repo.prototype.deleteLabel = function (opt_callback) {
    var instance = this,
        options = instance.options,
        payload;
...
Repo.prototype.listLabels = function (opt_callback) {
    var instance = this,
        options = instance.options,
        payload;
...
Repo.prototype.updateLabel = function(opt_callback) {
    var instance = this,
        options = instance.options,
        payload;
